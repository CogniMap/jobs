...
export class Controller implements ControllerInterface
{
   ...
   * TODO cache result ?
   */
  public generateWorkflow(generator : string, data : any, workflowId : string) : Workflow
  {
  ...
  {
    let self = this;
    return this.redis.setTaskStatus(workflowId, taskPath, 'queued').then(() => {
  ...
  {
    if (this.io != null) {
      this.io.sockets.in(workflowId).emit('setTasksStatuses', {
        id: workflowId,
        statuses
      });
    }
  ...
  public run(workflowId : string, path = '#', baseContext = {}, argument = null): Promise<any>
  {
    ...
    return this.redis.getWorkflow(workflowId)
      .then((workflowHash : WorkflowHash) => {
        let workflow = this.generateWorkflow(workflowHash.generator, workflowHash.generatorData, workflowId);
