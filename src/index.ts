...
export class Jobs
{
   ...
   */
  public createWorkflowInstance(workflowGenerator: string, workflowData: any, baseContext = {}, execute : boolean = false) : Promise<string>
  {
    let workflowId = uniqid();

    // Initialize the workflow instance in redis create tasks hashes
    let workflow = this.controller.generateWorkflow(workflowGenerator, workflowData, workflowId);
      ...
      socket.on('watchWorkflowInstance', function (workflowId) {
        this.redis.getWorkflow(workflowId)
          .then((workflowHash : WorkflowHash) => {
            ...
            let workflow = self.controller.generateWorkflow(workflowHash.generator, workflowHash.generatorData, workflowId);
